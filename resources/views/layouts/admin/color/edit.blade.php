...
<!-- end row -->
	<div class="row">
        <div class="col-lg-12">
			<form action="{{ url('admin/color/edit',$color->id)}}" method="POST" name="Add_color">
				@csrf
				<div class="card-box">
                    ...
                    <div class="form-group">
                        <label>Enter Color Name</label>
                            <input type="hidden" id="color_id" value="{{ $color->id }}">
                            <input type="text" name="name" id="name" parsley-trigger="change"
                                placeholder="Enter color name" value="{{ $color->name }}" class="form-control" class="@error('name') is-invalid @enderror">
                            @error('name')
